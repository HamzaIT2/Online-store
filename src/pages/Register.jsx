...
//   Alert, MenuItem, IconButton, InputAdornment, 
//   Avatar, Tooltip, Divider, Autocomplete 
// } from '@mui/material';
...
//   const getCurrentLang = () => i18n.language || 'ar';

//   // --- جلب البيانات (Provinces) ---
//   useEffect(() => {
//     // افترضنا وجود API لجلب المحافظات
//     const fetchProvinces = async () => {
//       try {
//         const res = await axios.get('http://localhost:3000/api/v1/provinces');
...
//   // --- جلب المدن عند تغيير المحافظة ---
//   useEffect(() => {
//     if (formData.province_id) {
//       const fetchCities = async () => {
//         try {
//           const res = await axios.get(`http://localhost:3000/api/v1/cities/${formData.province_id}`);
//           setCities(res.data);
//         } catch (err) {
...
//       setError(t('passwords_not_match') || 'كلمات المرور غير متطابقة');
//       setLoading(false);
//       return;
//     }

//     try {
//       // تنظيف البيانات وتحويل المعرفات لأرقام لتجنب خطأ 400
//       const payload = {
...
//       // التوجيه لصفحة تفعيل الكود (OTP) مع تمرير الإيميل
//       navigate('/verify-otp', { state: { email: formData.email } });

//     } catch (err) {
//       const serverResponse = err.response?.data;
//       if (serverResponse && Array.isArray(serverResponse.message)) {
//         // عرض مصفوفة الأخطاء (Array 2) بشكل نصي
//         setError(serverResponse.message.join(' | '));
//       } else {
//         setError(serverResponse?.message || 'حدث خطأ غير متوقع');
//       }
//     } finally {
//       setLoading(false);
//     }
...
//   // --- واجهة المستخدم (UI) ---
//   return (
//     <Container maxWidth="xs" sx={{ mt: 3, mb: 5, boxShadow: 3, borderRadius: 2, p: 3, bgcolor: 'background.paper' }}>
...
//           <Tooltip title={t('register_avatar') || 'صورة البروفايل'}>
//             <IconButton onClick={() => document.getElementById('register-avatar-input')?.click()}>
//               <Avatar src={avatarPreview || undefined} sx={{ width: 70, height: 70 }}>
//                 {formData.username?.charAt(0).toUpperCase() || 'U'}
//               </Avatar>
//             </IconButton>
//           </Tooltip>
//           <Box>
//             <Typography variant="body2">{t('register_avatar') || 'اختر صورة البروفايل'}</Typography>
//             <input id="register-avatar-input" hidden accept="image/*" type="file" onChange={handleFileChange} />
//             {avatarPreview && (
//               <Button size="small" color="error" onClick={handleRemoveAvatar}>{t('remove') || 'إلغاء'}</Button>
//             )}
//           </Box>
//         </Box>

//         <Divider sx={{ mb: 3 }}>{t('personal_info') || 'المعلومات الأساسية'}</Divider>

//         {/* تنبيه الأخطاء العام */}
//         {error && <Alert severity="error" sx={{ mb: 2 }}>{error}</Alert>}

//         <TextField
//           fullWidth required label={t('register_username')} name="username"
//           value={formData.username} onChange={handleChange} sx={{ mb: 2 }}
//           inputProps={{ maxLength: 20 }}
//         />

//         <TextField
//           fullWidth required label={t('register_full_name')} name="fullName"
//           value={formData.fullName} onChange={handleChange} sx={{ mb: 2 }}
//         />

//         <TextField
//           fullWidth required label={t('register_email')} name="email" type="email"
//           value={formData.email} onChange={handleChange} sx={{ mb: 2 }}
//         />

//         <TextField
//           fullWidth label={t('register_phone')} name="phone"
//           value={formData.phone} onChange={handleChange} sx={{ mb: 2 }}
//         />

//         <TextField
//           fullWidth required label={t('register_password')} name="password"
//           type={showPassword ? 'text' : 'password'}
...
//         <TextField
//           fullWidth required label={t('confirm_password') || 'تأكيد كلمة المرور'} name="confirmPassword"
//           type="password" value={formData.confirmPassword} onChange={handleChange} sx={{ mb: 2 }}
//         />

//         {/* اختيار المحافظة */}
//         <TextField
//           select fullWidth label={t('province')} name="province_id"
//           value={formData.province_id} onChange={handleChange} sx={{ mb: 2 }}
//         >
//           {provinces.map((p) => (
//             <MenuItem key={p.id} value={p.id}>
//               {getCurrentLang() === 'ar' ? p.nameAr : p.nameEn}
//             </MenuItem>
//           ))}
//         </TextField>
...
//         {/* اختيار المدينة */}
//         <TextField
//           select fullWidth label={t('city')} name="city_id"
//           value={formData.city_id} onChange={handleChange} sx={{ mb: 3 }}
//           disabled={!formData.province_id}
//         >
//           {cities.map((c) => (
//             <MenuItem key={c.id} value={c.id}>
//               {getCurrentLang() === 'ar' ? c.nameAr : c.nameEn}
//             </MenuItem>
//           ))}
//         </TextField>
...
//           disabled={loading} sx={{ fontWeight: 'bold' }}
//         >
//           {loading ? (t('loading') || 'جاري المعالجة...') : (t('register_title') || 'تسجيل')}
//         </Button>
//       </Box>
//     </Container>
//   );
